...
function trocarTema() {
    ...
    body.style.setProperty('--box-shadow', temaAtual === 'claro' ? '0px 5px 11px 0px rgba(0,0,0,0.2)' : '0px 5px 11px 0px rgba(255,255,255,0.2)')
    body.style.setProperty('--cor-background-hightlight', temaAtual === 'claro' ? 'white' : 'rgba(255, 255, 255, 0.212)')
    bntPrev.src = temaAtual === 'claro' ? './assets/seta-esquerda-preta.svg' : './assets/seta-esquerda-branca.svg'
    bntNext.src = temaAtual === 'claro' ? './assets/seta-direita-preta.svg' : './assets/seta-direita-branca.svg'
}
/*
...
input.addEventListener('keydown', (event) => {
    ...
    fetch('https://tmdb-proxy.cubos-academy.workers.dev/3/search/movie?language=pt-BR&include_adult=false&query=' + pesquisado)
        .then(response => {
            return response.json()
        })
        .then(responseFormat => {
